...
function template_manage_rom_games_list()
{
	...
	{
		echo '
					<option ' . (!empty($_SESSION['arcade_manage_rom_sort_select_alpha']) && $_SESSION['arcade_manage_rom_sort_select_alpha'] == $letter ? $selected : '') . 'value="' . $letter . '">' . sprintf($txt['arcade_manage_sort_alphabetical'], $letter) . '</option>';
	}
	...
	echo '
			...
			<div style="width: 50%;text-align: right;display: inline;padding-left: 3em;float: right;">
				<select id="arcadetypeval">
					<option value="0">' . $txt['arcade_manage_rom_sort_select_type'] . '</option>';

	foreach ($context['arcade_game_types_search_list'] as $key => $type)
	{
		echo '
					<option ' . (!empty($_SESSION['arcade_manage_rom_sort_select_type']) && $_SESSION['arcade_manage_rom_sort_select_type'] == $key && $key != 'all' ? $selected : '') . 'value="' . $key . '">' . $type . '</option>';
	}
	...
	if (!empty($context['qaction_title']))
	{
		...
		<form enctype="multipart/form-data" id="upload_form" action="', $scripturl, '?action=admin;area=manageromgames;sesc=', $context['session_id'], '" method="post" accept-charset="', $context['character_set'], '">
			<input type="hidden" name="sesc" value="', $context['session_id'], '" />
		</form>';
	}

	template_show_list('rom_games_list');
}

function template_manage_arch_replace()
{
	global $context, $txt;
	...
	<div class="centertext" id="romtimer" style="display: flex;align-items: center;justify-content: center;">
		<div class="timer" onload="timer(1800)" style="width: 10rem;height: 10rem;">
...
function template_manage_rom_install_games_list()
{
	global $context, $txt, $scripturl, $settings;
	$selected = $context['arcade_smf_version'] == '' ? 'selected ' : 'selected="selected" ';
	$alphaArray = array_merge(array('All'), range('A', 'Z'));
	...
	echo '
		<div style="padding: 2em 1em 2em 1em; width: 100%;">
			<div style="text-align: left;display: inline;padding-left: 3em;float: right;">
				<select id="arcadealphaval" onchange="arcadeRomLetter(this.value)">
					<option value="0">' . $txt['arcade_manage_sort_select_alpha'] . '</option>';

	foreach ($alphaArray as $letter)
	{
		echo '
					<option ' . (!empty($_SESSION['arcade_manage_sort_rom_select_alpha']) && $_SESSION['arcade_manage_sort_rom_select_alpha'] == mb_strtolower($letter) ? $selected : '') . 'value="' . $letter . '">' . sprintf($txt['arcade_manage_sort_alphabetical'], $letter) . '</option>';
	}
...
function template_manage_rom_games_upload()
{
	...
	<div class="windowbg2 upcontainer">
		...
		<div style="padding: 0.5em;" id="arcade_container">', (!empty($arcadeModSettings['arcadeUploadSystem']) ?	'
			<div style="padding: 4px 0px 4px 0px;font-family: tahoma;" class="mediumtext centertext">
				<div style="padding: 0.2rem 0.2rem 0.4rem 0.2rem;">' . $txt['arcade_supported_rom_filetypes'] . '</div>
				<div style="font-size: 12pt;overflow-wrap: break-word;">
					<div style="display: inline;border: 0px;padding: 2px;text-wrap: pretty;">' . ($context['rom_upload_file_types']) . '</div>
				</div>
			</div>
			<form id="upload" method="post" action="' . $scripturl . '?action=admin;area=manageromgames;sa=rom_upload2" enctype="multipart/form-data" accept-charset="' . $context['character_set'] . '">
				<div id="entire_upcon" style="overflow: hidden;border: 0px;margin: 0 auto;">
					<div id="upload_container">
						<div id="drop" class="centertext" style="height: 200px;padding-top: 30px;width: 100%;padding: 0px;margin: 0 auto;top: 50px;bottom: 50px;' . ($context['arcade_smf_version'] == 'v2.1' ? '' : 'position: relative;') . '">
							...
							<input style="display: none;" type="file" accept="' . $context['rom_file_types'] . '" name="upl" multiple />
						</div>
					</div>
			...
			<form id="upload_form" action="' . $scripturl . '?action=admin;area=manageromgames;sa=rom_upload2" method="post" enctype="multipart/form-data" accept-charset="' . $context['character_set'] . '">
				<input accept="' . $context['rom_file_types'] . '" type="file" size="48" name="attachment[]" /><br />
				<input accept="' . $context['rom_file_types'] . '" type="file" size="48" name="attachment[]" /><br />
				<input accept="' . $context['rom_file_types'] . '" type="file" size="48" name="attachment[]" /><br />
...
function template_manage_rom_games_uninstall_confirm()
{
	global $context, $txt, $scripturl, $settings;
...
function template_edit_rom_game_above()
{
	global $scripturl, $context, $txt;

	echo '
			<form action="', $scripturl, '?action=admin;area=manageromgames;sa=editrom2;sesc=', $context['session_id'], '" method="post" enctype="multipart/form-data">
...
function template_edit_rom_game_basic()
{
	global $scripturl, $context, $txt;
	...
	foreach ($context['arcade_category'] as $cat)
		echo '
												<option value="', $cat['id'], '"', $cat['id'] == $context['game']['category'] ? $selected : '', '>', $cat['name'], '</option>';
	echo '
											</select>
										</div>
									</div>';
	...
	echo '
									...
									<div style="display: table-row;">
										<div style="display: table-cell;"><label for="submit_system">', $txt['arcade_rom_system'], '</label></div>
										<div style="display: table-cell;">
											<select id="rom_system" name="rom_system">';

	foreach ($context['rom_systems'] as $system => $name)
		echo '
												<option value="', $system, '"', $context['game']['rom_system'] == $system ? $selected : '', '>', $name, '</option>';
	...
	echo '
									...
									<div style="display: table-row;font-size: 9pt;">
										<div style="display: table-cell;"><label for="external_link">', $txt['arcade_extra_options_link'], '</label></div>
										<div style="display: table-cell;">
											<input type="text" id="external_link" name="extra_data[external_link]" value="', (!empty($context['game']['extra_data']['external_link']) ? $context['game']['extra_data']['external_link'] : ''), '" style="width: 99%;"/>
										</div>
									</div>
									<div style="display: table-row;font-size: 8pt;">
										<div style="display: table-cell;"><label for="external_link_note">', $txt['arcade_emulator_note'], '</label></div>
										<div style="display: table-cell;"><div style="display: inline;">', $txt['arcade_emulator_symbolic'], '</div></div>
									</div>
									<div style="display: table-row;font-size: 9pt;">
										<div style="display: table-cell;">', $txt['arcade_extra_options_flash'], '</div>
										<div style="display: table-cell;">
											<div style="display: table-row;width: 100%;">
												<div style="display: table-row;">
													<div style="display: table-cell;width: 25%;margin-right: 0.5rem;">', $txt['arcade_extra_options_width'], ':</div>
												...
												<input type="hidden" name="extra_data[flash_version]" value="0" />
												<div style="display: table-row;">
									...
									</div>' . (pathinfo($context['game']['game_file'], PATHINFO_EXTENSION) != '7z' ? '
									<div style="display: table-row;">
										<div style="display: table-cell;"><label for="game_compression">' . (pathinfo($context['game']['game_file'], PATHINFO_EXTENSION) != 'zip' ? $txt['arcade_toggle_compression'] : $txt['arcade_toggle_decompression']) . '</label></div>
										<div style="display: table-cell;"><input type="checkbox" name="game_compression" id="game_compression"  value="1" /></div>
									</div>' : '');
// $context['game']['game_file']
}

function template_edit_rom_game_below()
{
	global $scripturl, $context, $txt;
	...
	$play = '<a class="arcadeLinkStyle" title="' . $txt['alt_play'] . '" href="' . $scripturl . '?action=retro_arch;sa=play;game=' . $context['game']['id'] . ';">' . $txt['arcade_admin_play'] . '</a>';
	...
	echo '
						...
						<div style="display: table-row;">
							<div style="display: table-cell;padding-top: 2em;text-align: left;padding-left: 0.6em;position: relative;">
								<a href="', $scripturl, '?action=admin;area=manageromgames;sa=uninstall;game=', $context['game']['id'], '">', $txt['arcade_uninstall'], '</a>
							...
							<div style="display: table-cell;padding-top: 2em;text-align: right;padding-right: 0.6em;position: relative;">
								...
								<a href="', $scripturl, '?action=admin;area=manageromgames;sa=romexport;game=', $context['game']['id'], '">', (sprintf($txt['game_info_export'], $context['game']['internal_name'] . '.php')), '</a>
							</div>
						</div>
					</div>
				</div>
