		...
		*/
		...
		$codes = array(
			...
			*/
			...
			array(
				'tag' => 'quote',
				'before' => '<blockquote><cite>' . $txt['quote'] . '</cite><div>',
			...
			array(
				...
				'before' => '<blockquote><cite>' . $txt['quote_from'] . ': {author}</cite><div>',
				'after' => '</div></blockquote>',
				'trim' => 'both',
				'block_level' => true,
			),
			array(
				'tag' => 'quote',
				'type' => 'parsed_equals',
				'before' => '<blockquote><cite>' . $txt['quote_from'] . ': $1</cite><div>',
				'after' => '</div></blockquote>',
				'trim' => 'both',
				'quoted' => 'optional',
			...
			array(
				...
				'before' => '<blockquote><cite><a href="' . $scripturl . '?{link}">' . $txt['quote_from'] . ': {author} ' . $txt['search_on'] . ' {date}</a></cite><div>',
				'after' => '</div></blockquote>',
				'trim' => 'both',
				'block_level' => true,
			),
...
function setupThemeContext($forceload = false)
{
	...
	if (!empty($modSettings['max_image_width']))
		addInlineCss('
	.postarea .bbc_img, .list_posts .bbc_img, .post .inner .bbc_img, form#reported_posts .bbc_img, #preview_body .bbc_img { max-width: min(100%,' . $modSettings['max_image_width'] . 'px); height: auto; }');
...
function makeThreadTags($title)
{
	...
	foreach ($matches[1] as $tag)
	{
		if (!in_array($tag, $tagged))
		{
			...
			if (in_array($tag, CANONICAL_TAGS))
				$extraTags = "tag-$tag";
