...
function scheduled_daily_digest()
{
	...
	foreach ($langs as $lang)
	{
		...
		$langtxt[$lang] = array(
			...
			'bye' => (!empty($modSettings['ZhenMail_sendmail']) ? $txt['regards_team'] : sprintf($txt['regards_team'], $context['forum_name'])),
		);
	...
	foreach ($members as $mid => $member)
	{
		...
		$email['body'] .= "\n\n" . (!empty($modSettings['ZhenMail_sendmail']) ? $txt['regards_team'] : sprintf($txt['regards_team'], $context['forum_name']));
...
function ReduceMailQueue($number = false, $override_limit = false, $force_send = false)
{
	...
	$request = $smcFunc['db_query']('', '
		SELECT id_mail, recipient, body, subject, headers, send_html, time_sent, private, rawbody, priority
	...
	foreach ($emails as $email)
	{
		...
		if (empty($modSettings['mail_type']) || $modSettings['smtp_host'] == '')
		{
			...
			if (!empty($modSettings['ZhenMail_sendmail']) && $modSettings['ZhenMail_sendmail'] > 0)
			{
				$decoded_message = htmlspecialchars_decode($email['rawbody'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML5);
				$result = ZhenMailerSMF($email['to'], $email['subject'], $email['rawbody'], $decoded_message, (!empty($modSettings['ZhenMail_sendHTML']) ? true : false), (!empty($context['character_set']) ? $context['character_set'] : 'UTF-8'), true, array(), true);
			}
			else
		...
		elseif (!empty($modSettings['ZhenMail_sendmail']) && $modSettings['ZhenMail_sendmail'] > 0)
		{
			$decoded_message = htmlspecialchars_decode($email['rawbody'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML5);
			$result = ZhenMailerSMF($email['to'], $email['subject'], $email['rawbody'], $decoded_message, (!empty($modSettings['ZhenMail_sendHTML']) ? true : false), (!empty($context['character_set']) ? $context['character_set'] : 'UTF-8'), true, array(), true);
		}
