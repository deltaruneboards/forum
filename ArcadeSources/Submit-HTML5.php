...
function ArcadeHTML5Game()
{
	...
	if ($initialToken[1] == $gameToken)
	{
		...
		if (empty($cheating) && empty($_SESSION['arcade_check_' . $context['game']['id']]))
		{
			...
			echo '<script type="text/javascript">window.location.replace("' . $url . '");</script>';
			if (isset($_POST['popup']) && $_POST['popup'] == 1)
				redirectexit($scripturl . '?action=arcade;end;sa=highscore;pop=1;game=' . $gameid. ';#commentform3');
			else
				redirectexit($scripturl . '?action=arcade;end;sa=highscore;game=' . $gameid. ';#commentform3');
...
function ArcadeHTML5Html(&$game, $auto_start = true)
{
	...
	<div style="display: flex;align-items: center;justify-content: center;">
		<div id="gameplayer" style="overflow: hidden;margin: auto; width: ', $game['width'], 'px; height: ', $game['height'], 'px; ">
			', $txt['arcade_html5'], '
		</div>
	</div>
	...
	<script type="text/javascript"><!-- // --><![CDATA[
		...
		function ArcadeStart()
		{
			...
			var so = \'<object id="arcadeGameObj" type="text/html" style="overflow: hidden;height: 100vh;width: 100vw;position: absolute;top: 0px;left: 0px;" data="' . $arcadeModSettings['gamesUrl'] . '/' . $game['directory'] . '/' . $game['file'] . '"></object>\';' : '
