...
class SWFReader
{
	...
	function open($file)
	{
		...
		$this->header['background'] = !empty($bg['data']) ? $bg['data'] : '#000000';
		unset($bg);
	}
	...
	{
		// This may use a lot of memory due to compressed files
		...
		if ($memory_limit > 0)
		{
			...
			if ($this->header['file_length'] * $multi > $memory_limit)
				@ini_set('memory_limit', ($this->header['file_length'] * $multi) . 'M');
		}
	}
